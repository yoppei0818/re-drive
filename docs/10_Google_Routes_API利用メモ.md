...
### Phase 0の利用量確認（2026年8月30日）

Re:DriveプロジェクトのGoogle Cloud Consoleで、開発中の実測値を確認しました。

| 項目 | 確認結果 |
| --- | --- |
| 確認期間 | 直近1日（指標）、2026年8月1日〜28日（請求） |
| Routes APIリクエスト数 | 1件 |
| エラー | なし |
| 平均レイテンシ | 39 ms |
| 99パーセンタイルレイテンシ | 65 ms |
| 当月費用 | 0円 |
| `ComputeRoutes`の日次クォータ | 無制限、現在の使用量1件、画面上は調整不可 |
| `ComputeRoutes`の毎分クォータ | 3,000件、現在の使用量0件、画面上は調整不可 |

請求レポートは費用0円でSKU別の明細がまだ表示されていないため、実請求明細からのSKU確認は利用データ反映後に再確認します。実装は`computeRoutes`、`TRAFFIC_UNAWARE`、基本のPolyline、通常の経由地を使用しているため、公式のSKU判定上は`Routes: Compute Routes Essentials`（SKU ID `9EFF-679A-9B16`）を意図した構成です。

Google Cloud Consoleでは現在のクォータを確認できましたが、表示された`ComputeRoutes`クォータは調整不可でした。そのため、Phase 0では予算アラートを主要な費用監視手段とし、公開前にFastAPI側のrate limitも検討します。

### 1回のルート生成で発生するリクエスト

`POST /api/v1/routes/preview`を1回実行すると、FastAPIから`computeRoutes`を1回呼び出します。Maps URLの生成とGoogle Mapsアプリへの遷移ではRoutes APIを追加で呼びません。

```text
Routes API呼び出し数 = プレビュールート生成回数 + retry回数
```

現在は自動retryを実装していないため、通常は「1回のルート生成 = 1回の課金対象リクエスト」です。モバイルから再試行した場合は、新しい1回として加算されます。

### 月額概算

2026年8月時点の`Compute Routes Essentials`は月10,000回まで無料で、10,001〜100,000回の部分は1,000回あたり5米ドルです。以下では、1ユーザーが月10回ルートを生成する前提を置きます。

| 利用規模 | 月間ルート生成数 | 月間APIリクエスト数 | 概算料金 |
| --- | ---: | ---: | ---: |
| 開発中 | 100回 | 100回 | 0米ドル |
| 月100ユーザー | 100人 × 10回 = 1,000回 | 1,000回 | 0米ドル |
| 月1,000ユーザー | 1,000人 × 10回 = 10,000回 | 10,000回 | 0米ドル |

無料枠を超えた場合の概算式は次のとおりです。実際の請求では、同じ請求先アカウントに紐づくプロジェクトのSKU利用量が集約され、税、為替、契約条件も影響します。

```text
月額（米ドル） = max(月間リクエスト数 - 10,000, 0) / 1,000 × 5
```

例として月1,000ユーザーが1人あたり月20回生成すると20,000回となり、概算は50米ドルです。1米ドル150円で参考換算すると約7,500円ですが、予算管理では実際の請求通貨と為替を使用します。

### Phase 0の費用管理方針

- Re:Driveプロジェクトを対象に月間の初期予算1,000円を設定した
- 予算の50%、80%、100%でメール通知するアラートを設定した
- 予算アラートはAPI停止や請求上限ではなく、通知のみであることに注意する
- 無料枠内でも利用量とSKUを月1回確認する
- 本番公開前に利用頻度の仮定を実測値へ置き換える
